...
class ContractContract(models.Model):
    ...
    stock_count = fields.Integer(compute="_compute_invoice_count")
    ...
    type_of_contract = fields.Many2one('contract.type')

    contract_total_remaining = fields.Monetary(string='Total invoice remaining', store=True, readonly=True,
                                               compute='_compute_remaining', tracking=4)
    contract_total_invoice = fields.Monetary(string='Total invoiced', store=True, readonly=True,
                                             compute='_compute_remaining', tracking=4)
    ...
    account_ids = fields.One2many('account.move', 'old_contract_id')
    stock_picking_ids = fields.One2many('stock.picking', 'contract_id')
    commercial_registration_number = fields.Char('Commercial Registration Number')

    def show_picking(self):
        for rec in self:
            res = self.env.ref('stock.action_picking_tree_all')
            res = res.read()[0]
            res['domain'] = str([('contract_id', '=', rec.id)])
        return res
    ...
    def action_confirm(self):
        ...
        for rec in self:
            rec.state = 'confirm'
        # for record in self:
        #     contract_info = {'name': record.name, 'method_period':'12'if record.recurring_rule_type == 'monthly' else '12',
        ...
        #                      'journal_id': record.journal_id.id, 'account_depreciation_id': record.type_of_contract.income_account.id,'asset_type':'sale',
    ...
    @api.depends('account_ids.invoice_line_ids.quantity')
    def _compute_remaining(self):
        ...
        for record in self:
            ...
            record.contract_total_invoice = total
            record.contract_total_remaining = record.contract_total - total
    ...
    def _compute_invoice_count(self):
        for rec in self:
            ...
            rec.stock_count = len(self.env["stock.picking"].search([("contract_id", "=", self.id)])
                                  )
    ...
    def get_count_list(self):
        total_contract = self.env['contract.contract'].sudo().search_count([])
        new_contract = self.env['contract.contract'].sudo().search_count([('state', '=', 'new')])
        running_contract = self.env['contract.contract'].sudo().search_count([('state', '=', 'running')])
        confirm_contract = self.env['contract.contract'].sudo().search_count([('state', '=', 'confirm')])
        stop_contract = self.env['contract.contract'].sudo().search_count([('state', '=', 'stop')])
        total_delivered = self.env['stock.picking'].sudo().search_count([])
...
class ContractType(models.Model):
    ...
    #     return Accounts.search([], limit=1).id
