...
class ContractLine(models.Model):
    ...
    qty_delivered = fields.Float(
        compute='_compute_qty_delivered', string='Delivered Quantity', store=True,
        digits='Product Unit of Measure')

    @api.depends('contract_id.account_ids.invoice_line_ids.quantity')
    def _compute_qty_invoiced(self):
        total = 0
        for record in self:
            for rec in record.contract_id.account_ids:
                for r in rec.invoice_line_ids:
                    if record.product_id == r.product_id:
                        total += r.quantity
            record.qty_invoiced = total

    @api.depends('contract_id.stock_picking_ids.move_ids_without_package.quantity_done')
    def _compute_qty_delivered(self):
        total = 0
        for record in self:
            for rec in record.contract_id.stock_picking_ids:
                for r in rec.move_ids_without_package:
                    if record.product_id == r.product_id:
                        total += r.quantity_done
            record.qty_delivered = total
