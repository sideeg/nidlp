...
function makeActionManager(env) {
    ...
    function _getSwitchViewParams() {
        ...
        if (state.action && !actionRegistry.contains(state.action)) {
            const currentController = controllerStack[controllerStack.length - 1];
            const currentActionId = currentController && currentController.action && currentController.action.id;
    ...
    const WKHTMLTOPDF_MESSAGES = {
        broken:
            env._t("Your installation of Wkhtmltopdf seems to be broken. The report will be shown " + "in html.") +
            link,
        install: env._t("Unable to find Wkhtmltopdf on this system. The report will be shown in " + "html.") + link,
        upgrade:
            env._t(
        ...
        workers: env._t("You need to start Odoo with at least two workers to print a pdf version of " + "the reports."),
    };
    ...
    async function _executeReportAction(action, options) {
        ...
        if (action.report_type === "qweb-html") {
            ...
            //CUSTOM CODE
            return _triggerDownload(action, options, "xlsx"); //CUSTOM CODE
        } else {
    ...
    async function doActionButton(params) {
        ...
        if (params.special) {
            ...
            action = action && typeof action === "object" ? action : {type: "ir.actions.act_window_close"};
    ...
    async function switchView(viewType, props = {}) {
        ...
        Object.assign(newController, _getViewInfo(view, controller.action, controller.views, props));
    ...
    async function restore(jsId) {
        ...
        if (controller.action.type === "ir.actions.act_window") {
            Object.assign(controller, _getViewInfo(controller.view, controller.action, controller.views));
        }
        await clearUncommittedChanges(env);
